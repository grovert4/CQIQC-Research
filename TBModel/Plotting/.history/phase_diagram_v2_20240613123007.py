...
im = ax.imshow(polarization[:,:Jidx], aspect='auto', cmap='PuBuGn', origin='lower',
               extent=[J_array.min(), J_array[Jidx], U_array.min(), U_array.max()])
ax.set_ylabel(r'$U$')
ax.set_xlabel(r'$J$')
...
im_ins = axins.imshow(conduct[:,:Jidx], aspect='auto', cmap='PRGn',vmin = -2, vmax=2, origin='lower',
                      extent=[J_array.min(), J_array[Jidx], U_array.min(), U_array.max()])
cax_1 = inset_axes(ax,
