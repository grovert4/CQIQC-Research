...
outloc = "/media/andrewhardy/9C33-6BBD/Skyrmion/TBModel/Plotting/Plots/SSF/"
...
for p in ps
    for U in Us
        ...
        savefig("./TBModel/Plotting/Plots/SSF/$(date)_U=$(U)_p=$(p)_NSF_$(layer).png")

        RSPlot = plot_monolayer_Sz(UC, polarizations)
        savefig(RSPlot, "./TBModel/Plotting/Plots/SSF/$(date)_U=$(U)_p=$(p)_RS_$(layer).png")
    end
end