...
filename = "08.01-25.2024_Bilayer"
...
for (ind_V,V) in enumerate(V_array):
    for (ind_u, U_var) in enumerate(U_array):
        ...
        try:
            ...
            #polarization[ind_u, ind_V] = np.abs(np.sum(TBResults["Expectations"][-12:] - TBResults["Expectations"][-24:-12]))
...
# Assuming V_array, U_array, polarization, conduct, and filename are already defined
...
Vidx = np.searchsorted(V_array, 0.4)
...
im1 = ax1.pcolormesh(X, Y, polarization[:,:Vidx]/12, cmap=new_cmap, vmin=0, vmax=0.05, shading='auto')
ax1.set_ylabel(r'$U$', fontsize=30)
ax1.set_xlabel(r'$V$', fontsize=30)
ax1.set_ylim(U_array.min(), min(U_array.max(), 7))
...
im2 = ax2.pcolormesh(X, Y, np.abs(conduct[:,:Vidx]), cmap='Purples', vmin=0, vmax=1, shading='auto')
ax2.set_xlabel(r'$V$', fontsize=30)
ax2.set_ylim(U_array.min(), min(U_array.max(), 7))
...
im = ax.pcolormesh(X, Y, polarization[:,:Vidx]/12, cmap=new_cmap, vmin=0, vmax=0.521, shading='auto')

ax.set_ylabel(r'$U$')
ax.set_xlabel(r'$V$')
ax.set_ylim(U_array.min(), min(U_array.max(), 7))
# Inset plot
axins = inset_axes(ax, width="49%", height="49%", loc='lower right', bbox_to_anchor=(-0.05, 0.05,0.95, 1.05 ),bbox_transform=ax.transAxes)
#im_ins = axins.imshow(np.abs(conduct[:,:Vidx]), aspect='auto', cmap='PuBuGn',vmin = 0, vmax=2, origin='lower',
#                      extent=[V_array.min(), V_array[Vidx], U_array.min(), U_array.max()])
im_ins = axins.pcolormesh(X, Y, np.abs(conduct[:,:Vidx]), cmap='Purples', vmin=0, vmax=1, shading='auto')
#shading='gouraud'
...
#plt.colorbar(im, cax=cax_1, label=r'$N(Q=0)$')
plt.colorbar(im, cax=cax_1, label=r'$\Delta n$')
...
plt.plot(U_array[:Uidx], np.abs(conduct[:Uidx,0]), linewidth = 2.5, markersize = 8,marker = "o", label = r"$|\sigma_{xy}|$", color = colors[3], markeredgecolor='black', markeredgewidth=0.65)
plt.plot(U_array[:Uidx], p[:Uidx]/12*10, linewidth = 2.5, markersize = 10,marker = "X", label = r"$10 \times\Delta n$", color = colors[1], markeredgecolor='black', markeredgewidth=0.65)
plt.plot(U_array[:Uidx], 100*ssf[:Uidx,0], linewidth = 2.5, markersize = 8,marker = "^", label = r"$100 \times \Delta n(Q)$", color = colors[2], markeredgecolor='black', markeredgewidth=0.65)
