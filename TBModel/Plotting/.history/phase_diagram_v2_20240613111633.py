...
for (ind_J,J) in enumerate(J_array):
    for (ind_u, U_var) in enumerate(U_array):
        ...
        if (polarization[ind_u, ind_J] < 0.06) and (J_array[ind_J] < -1.7):
...
# Main plot
im = ax.imshow(np.flip(polarization), aspect='auto', cmap='PuBuGn', origin='lower',
               extent=[J_array.min(), J_array.max(), U_array.min(), U_array.max()])
ax.set_ylabel(r'$U$')
ax.set_xlabel(r'$J$')
...
# Inset plot
axins = inset_axes(ax, width="45%", height="45%", loc='lower right',bbox_to_anchor=(0.05, 0.05, 1.05, 1.05), bbox_transform=ax.transAxes)
im_ins = axins.imshow(np.flip(conduct), aspect='auto', cmap='PRGn',vmin = -2, vmax=2, origin='lower',
                      extent=[J_array.min(), J_array.max(), U_array.min(), U_array.max()])
...
plt.savefig("Plots/"+filename+"_U_J.pdf")
