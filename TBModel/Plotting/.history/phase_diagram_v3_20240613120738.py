...
# Main plot
im = ax.pcolormesh(polarization, aspect='auto', cmap='PuBuGn', origin='lower',
               extent=[V_array.min(), 1.0, U_array.min(), U_array.max()])
ax.set_ylabel(r'$U$')
ax.set_xlabel(r'$V$')
...
im_ins = axins.pcolormesh(conduct, aspect='auto', cmap='PRGn',vmin = -2, vmax=2, origin='lower',
                      extent=[V_array.min(), 1.0, U_array.min(), U_array.max()])
cax_1 = inset_axes(ax,
