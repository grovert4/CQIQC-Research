...
function MFT(params, filename)
    ...
    GC.gc()
    rand_noise = rand(SkXSize^2 * 9) .- 0.5
    rand_noise = 0.05 .* (rand_noise .- sum(rand_noise) / (SkXSize^2 * 9))

    init_up = fill(filling, SkXSize^2 * 9) .+ rand_noise .- 0.06
    init_dn = fill(filling, SkXSize^2 * 9) .- rand_noise .+ 0.06
