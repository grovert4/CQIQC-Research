...
function MFT(params, filename)
    ...
    for (ind,bond) in enumerate(t1Param.unitBonds)
        ...
        AddAnisotropicBonds!(hopping_up[ind], UC, bond.base, bond.target,bond.offset, n_top,bond.dist, "texp-" * string(ind))
    end
    for (ind,bond) in enumerate(t1Param.unitBonds)
        push!(hopping_up, Param(1.0, 2))
        AddAnisotropicBonds!(hopping_up[ind], UC, bond.base, bond.target,bond.offset, n_bottom,bond.dist, "texp-" * string(ind))
    end
