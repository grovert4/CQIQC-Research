...
function MFT(params, filename)
    ...
    ChiParams = vcat(Density_up, Density_dn)
    ...
    init_up = fill(filling, SkXSize^2 * 3) .+ rand_noise .- 0.25
    init_dn = fill(filling, SkXSize^2 * 3) .- rand_noise .+ 0.25
    #init_guess = vcat(fill(0.0,length(hopping_up)*2), fill(0.00,SkXSize^2 * 3), init_up, init_dn)
    init_guess = vcat(init_up, init_dn)
    ...
    if isfile(fileName)
        ...
        try
            ...
            if haskey(params, "U_prev")
                ...
                init_guess = load(oldfile)["outputs"][end] .+ vcat( rand_noise, -1 .* rand_noise)
