...
function MFT(params, filename)
    ...
    for (ind,bond) in enumerate(t1Param.unitBonds)
        ...
        AddAnisotropicBonds!(hopping[ind], UC, bond.base, bond.target,bond.offset, su2spin[4],bond.dist, "texp-" * string(ind))
    end
    for (ind, bas) in enumerate(UC.basis)
    ...
    if isfile(fileName)
        ...
        try
            ...
            if haskey(params, "U_prev")
                ...
                init_guess = load(oldfile)["outputs"][end] .+ vcat(fill([0.001],SkXSize*2), rand_noise)
        ...
        if haskey(params, "U_prev")
            ...
            init_guess = load(oldfile)["outputs"][end] .+ vcat(fill([0.0001],SkXSize*2), rand_noise)
            SolveMFT!(mft, init_guess, fileName; max_iter=params["max_iter"], tol=params["tol"])
        else
            SolveMFT!(mft, init_guess, fileName; max_iter=params["max_iter"], tol=params["tol"])
        end
