...
im = ax.pcolormesh(X, Y, polarization[:,:Vidx], cmap=new_cmap, vmin=0, vmax=0.521, shading='auto')
...
plt.colorbar(im, cax=cax_1, label=r'$\Delta\overline{n}$')
...
plt.plot(U_array[:Uidx], np.abs(conduct[:Uidx,0]), linewidth = 2.5, markersize = 8,marker = "o", label = r"$|\sigma_{xy}|$", color = colors[3])
plt.plot(U_array[:Uidx], p[:Uidx], linewidth = 2.5, markersize = 8,marker = "X", label = r"$\Delta\overline{n}$", color = colors[1])
plt.plot(U_array[:Uidx], 10*ssf[:Uidx,0], linewidth = 2.5, markersize = 8,marker = "^", label = r"$10 \times \Delta n(Q)$", color = colors[2])
