...
for (ind_V,V) in enumerate(V_array):
    for (ind_u, U_var) in enumerate(U_array):
        ...
        try:
            ...
            if polarization[ind_u, ind_V] < 0.005:
...
new_cmap = LinearSegmentedColormap.from_list("Blues_with_white", colors)

im = ax.pcolormesh(X, Y, polarization[:Uidx,:Vidx], cmap='Blues', vmin=0, vmax=0.521, shading='auto')
