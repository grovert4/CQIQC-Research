...
ax.xaxis.set_minor_locator(mpl.ticker.AutoMinorLocator(2))
ax.yaxis.set_minor_locator(mpl.ticker.AutoMinorLocator(2))
p = polarization[u_idx,10:]#/np.max(polarization[0,:])
s = ssf[u_idx,10:]
print(U_array[u_idx])
...
plt.plot(V_array[10:], np.abs(conduct[u_idx,10:]), linewidth = 2.5, markersize = 8,marker = "o", label = r"$|\sigma_{xy}|$", color = colors[3])
plt.plot(V_array[10:], p, linewidth = 2.5, markersize = 8,marker = "X", label = r"$\Delta \overline{n}$",color = colors[1])
plt.plot(V_array[10:], s, linewidth = 2.5, markersize = 8,marker = "^", label = r"$n(Q=K)$", color = colors[2])
