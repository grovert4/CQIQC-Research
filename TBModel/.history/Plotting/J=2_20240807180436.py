...
plt.colorbar(im, cax=cax_1, label=r'$\Deltan(Q)$')
...
plt.plot(V_array[v_idx:], np.abs(conduct[u_idx,v_idx:]), linewidth = 2.5, markersize = 8,marker = "o", label = r"$|\sigma_{xy}|$", color = colors[3])
plt.plot(V_array[v_idx:], p, linewidth = 2.5, markersize = 8,marker = "X", label = r"$\Delta \overline{n}$",color = colors[1])
plt.plot(V_array[v_idx:], s, linewidth = 2.5, markersize = 8,marker = "^", label = r"$\Deltan(Q)$", color = colors[2])
