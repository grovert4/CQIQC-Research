...
filling_arr = np.linspace(params["filling_min"], params["filling_max"], params["filling_length"]) / (params["filling_max"] )
#filling_array = np.linspace(params["filling_min"], params["filling_max"], params["filling_length"])
...
for (ind_V,V) in enumerate(filling_array):
...
im = ax.imshow(polarization[:,:Vidx], aspect='auto', cmap='PuBuGn', origin='lower',
               extent=[filling_array.min(), filling_array[Vidx], U_array.min(), U_array.max()])
ax.set_ylabel(r'$U$')
ax.set_xlabel(r'$n$')
