...
function GeoTensor(Ham::Hamiltonian, bz::BZ, mu::Float64, subset::Vector{Int64}, 1, 2)
        ...
        if ind == 1 || ind == length(Es)
            ...
            for (g, i) in enumerate(subset)
                ...
                for j in ind:length(Es)
                    if !(j in subset)
                        geotensor[k] += (vy[i, j] * vy[j, i] ) / ((Es[j] - Es[i])^2)
                    end   
                end
            end
        end
...
display(ssf_plot)
GetVelocity!(H, bz)
geo = GeoTensor(H, bz, Mdl.mu, [13])# #for (i, filling) in enumerate(filling_arr)
...
imag(sum(geo)) * bzUnitArea * 2 * pi / length(H.bands)
(1/(2*pi)) * sum(berry)
