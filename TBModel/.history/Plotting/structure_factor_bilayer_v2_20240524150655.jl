...
const n = 12#
const n_max = 12
#const ps = round.(collect(range(1, n_max, n) / (2 * n_max)), digits=3)
const Js = round.(collect(LinRange(-4.0, 0.0, 10)), digits=3)
...
for J in Js
    for U in Us
        ...
        filename = directory * "Last_Itr_$(date)_$(layer)_J=$(J)_U=$(U).jld2"
        #filename = directory * "Last_Itr_$(date)_$(layer)_E_p=$(J)_U=$(U)_t1=$(t1).jld2"
        #filename = directory * "Last_Itr_$(date)_$(layer)_4_p=$(J)_U=$(U)_t1=$(t1).jld2"
        ...
        elseif layer == "Bilayer"
            len = length(data["Expectations"][3:end])
            polarizations = data["Expectations"][3:div(len, 2)+2] - data["Expectations"][3+div(len, 2):end]
        end
        ...
        savefig(outloc * "/Plotting/Plots/SSF/$(date)_U=$(U)_p=$(J)_NSF_$(layer).png")

        RSPlot = plot_monolayer_Sz(UC, polarizations)
        savefig(RSPlot, outloc * "/Plotting/Plots/SSF/$(date)_U=$(U)_p=$(J)_RS_$(layer).png")
    end
end