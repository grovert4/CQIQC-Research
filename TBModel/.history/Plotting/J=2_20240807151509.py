...
for (ind_V,V) in enumerate(V_array):
    for (ind_u, U_var) in enumerate(U_array):
        ...
        try:
            ...
            polarization[ind_u, ind_V] = np.abs(ssf_up)-ssf_dn)
            # if V < 0.3:
...
im = ax.pcolormesh(X, Y, polarization[:Uidx,:Vidx]/np.max(polarization), cmap='Purples', vmin=0, vmax=1.0, shading='auto')
...
#                      extent=[V_array.min(), V_array[Vidx], U_array.min(), U_array.max()])
im_ins = axins.pcolormesh(X, Y, np.abs(conduct[:Uidx,:Vidx]), cmap='Blues', vmin=0, vmax=1, shading='auto')
#shading='gouraud'
...
plt.plot(V_array, np.abs(conduct[u_idx,:]), linewidth = 2, markersize = 10,marker = "o", label = r"$|\sigma_{xy}|$")
