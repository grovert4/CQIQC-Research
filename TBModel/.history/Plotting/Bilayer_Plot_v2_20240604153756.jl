...
for (ind, U_var) in enumerate(U_array[:])
    ...
    len = length(TBResults["Expectations"][3:end])
    order_parameter[ind, :] = TBResults["Expectations"][3:div(len, 2)+2] .- TBResults["Expectations"][div(len, 2)+3:end]
    #println(length(ords))
    ord_array[ind, :] = TBResults["Expectations"][3:end]#(mean(abs.(ords)))
...
RSPlot = plot_RS(UC, ord_array[10, 1:SkXSize^2*3] .- ord_array[10, SkXSize^2*3:ord_array[10, 1:SkXSize^2*6]])
display(RSPlot)
