...
function MFT(params, filename)
    ...
    for (ind,bond) in enumerate(t1Param.unitBonds)
        push!(hopping, Param(1.0, 2))
        AddAnisotropicBond!(hopping[ind], UC, bond.base, bond.target,bond.offset, su2spin[4],bond.dist, "texp-" * string(ind))
    end
    for (ind, bas) in enumerate(UC.basis)
    ...
    init_guess = vcat(fill([1.0],length(hopping)), fill(filling, SkXSize^2 * 3) .+ rand_noise)   # some random # which
    if isfile(fileName)
        ...
        try
            ...
            if haskey(params, "U_prev")
                ...
                init_guess = load(oldfile)["outputs"][end] .+ vcat(fill([0.001],SkXSize^2 * 3), rand_noise)
