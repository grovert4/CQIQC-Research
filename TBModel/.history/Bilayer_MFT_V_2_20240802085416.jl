...
function MFT(params, filename)
    ...
    U = get!(params, "U", 0.0)
    t_density = get!(params, "t_density", 0.0) # add Weiss field
    ...
    for (ind,bond) in enumerate(t1Param.unitBonds)
        ...
        AddAnisotropicBond!(hopping_up[ind], UC, bond.base, bond.target,bond.offset, n_top,bond.dist, "t_up-" * string(ind))
    end
    for (ind,bond) in enumerate(t1Param.unitBonds)
        push!(hopping_dn, Param(1.0, 2))
        AddAnisotropicBond!(hopping_dn[ind], UC, bond.base, bond.target,bond.offset, n_bottom,bond.dist, "t_dn-" * string(ind))
    end
    ...
    init_guess = load(oldfile)["outputs"][end] .+ vcat(fill(0.000,length(hopping_up)*2), fill(0.000,SkXSize^2 * 3), init_up,  init_dn)
        ...
        try
            ...
            if haskey(params, "U_prev")
                ...
                #oldfile = loc * "/$(filename)_J=$(round(jh, digits=3))_U=$(round(params["U_prev"], digits=2)).jld2"
                    ...
                    init_guess = load(oldfile)["outputs"][end] .+ vcat(fill(0.00001,length(hopping_up)*2), fill(0.0001,SkXSize^2 * 3), rand_noise,  -1*rand_noise)
                end
