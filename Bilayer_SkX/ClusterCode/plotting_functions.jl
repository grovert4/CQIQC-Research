...
function plotMonoReadSpins(file, lat, vertex)
...
function plotBiReadSpins(file, lat, vertex = none, bg=true)
    updateSpins!(file,lat)
    PyPlot.isjulia_display[] = false
    xpos,ypos = [],[]
    for i in 1:2:length(lat.sitePositions)
...
function plotSzLine(lat, siteInit, factor)
    site = siteInit
    ind = 1
    v = lat.sitePositions[siteInit + face] .- lat.sitePositions[siteInit]
    xspin1,yspin1,zspin1 = [],[],[]
    while checkbounds(Bool, tempLattice.sitePositions, site)
        push!(xspin1,lat.spins[1,site])
        push!(yspin1,lat.spins[2,site])
        push!(zspin1,lat.spins[3,site])
        site += factor
        ind += 1
    end
    Plots.quiver(collect(1:length(xspin1)), repeat([0],length(xspin1)),quiver=(xspin1,zspin1), legend=false)
end
