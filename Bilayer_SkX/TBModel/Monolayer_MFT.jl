...
AddIsotropicBonds!(UParam, InteractionUC, 1.0, U, "Hunds Interaction")
InteractionBlock   =  ParamBlock([UParam], InteractionUC)
    
##Adding inner-hexagon structure  
for j = 1:2
    for i = -1:4
        AddBasisSite!(HoppingUC, i .* l1 + j .* l2)
        AddBasisSite!(InteractionUC, i .* l1 + j .* l2)
    end
end
