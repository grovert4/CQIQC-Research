...
for U in U_array
    ...
    for (ind, bas) in enumerate(UC.basis)
        ...
        AddAnisotropicBond!(Sx, UC, ind,ind, [ 0, 0],  SpinVec[1], 0.0, "Sx order")
        AddAnisotropicBond!(Sy, UC, ind,ind, [ 0, 0],  SpinVec[2], 0.0, "Sy order")
        AddAnisotropicBond!(Sz, UC, ind,ind, [ 0, 0],  SpinVec[3], 0.0, "Sz order")

    end
    ChiParams    =  [t_s, Sx,Sy,Sz]
    ...
    fileName    =   "./Monolayer/Monolayer=$(round(filling, digits=3))_U=$(round(U, digits=2))_t1=$(round(t1, digits=2)).jld2"
    ...
    display(bands)
    savefig(bands, "MonLayer_MFT_Results_U=$(round(U, digits=2)).png")
