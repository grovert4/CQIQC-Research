...
for (ind, bas) in enumerate(UC.basis)
    if 1 < norm(bas) < 2
        ...
        mat = intermat(tau( closest[findmin(x -> norm(x), closest)[2]] ))
    end
...
println("Chern Number for last 12 bands: ", ChernNumber(H, collect(13:24)))
println("Overall Chern Number", ChernNumber(H, collect(1:24)))
