...
const layer = "Monolayer"
const directory = "./$(layer)_data/"
...
for p in ps
    for U in Us
        ...
        filename = directory * "Last_Itr_$(date)_$(layer)_NN_p=$(p)_U=$(U)_t1=$(t1).jld2"
        data = load(filename)
        println("data loaded")

        if layer == "Monlayer"
            polarizations = data["Order_Parameter"][2:end]
        elseif layer == "Bilayer"
            polarizations = data["Order_Parameter"]
        end
        ...
        savefig("$(date)_U=$(U)_p=$(p)_NSF_monolayer.png")

        RSPlot = plot_monolayer_Sz(UC, polarizations)
        savefig(RSPlot, "$(date)_U=$(U)_p=$(p)_monolayer_RS.png")
    end
end
